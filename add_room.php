...
$page_url = new moodle_url('/mod/skyroom/add_room.php', array('id' => $cm->id));
$PAGE->set_url('/mod/skyroom/add_room.php', array('id' => $cm->id));
...
foreach($users as $user) {
    try {
        $skyroom_user = skyroom_createUser3($user);
        if(has_capability('moodle/course:update', $context_course, $user)) {
            //admin and editing teacher
            $result = skyroom_addRoomUser($skyroom->room_id, $skyroom_user->skyroom_user_id, 3);
            if(!$result || !$result->ok) {
                skyroom_UpdateRoomUser($skyroom->room_id, $skyroom_user->skyroom_user_id, 3);
            }
        } else if(skyroom_isTeacher($course)) {
            //teacher
            $result = skyroom_addRoomUser($skyroom->room_id, $skyroom_user->skyroom_user_id, 2);
            if(!$result || !$result->ok) {
                skyroom_UpdateRoomUser($skyroom->room_id, $skyroom_user->skyroom_user_id, 2);
            }
        } else {
            //student
            $result = skyroom_addRoomUser($skyroom->room_id, $skyroom_user->skyroom_user_id, 1);
            if(!$result || !$result->ok) {
                skyroom_UpdateRoomUser($skyroom->room_id, $skyroom_user->skyroom_user_id, 1);
            }
        }
        $ok_user[] = $user;
    } catch (Exception $e) {
