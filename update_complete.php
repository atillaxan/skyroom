...
$page_url = new moodle_url('/mod/skyroom/update_complete.php', array('id' => $cm->id));
$PAGE->set_url('/mod/skyroom/update_complete.php', array('id' => $cm->id));
...
foreach($users as $user) {
    try {
        ...
        if(has_capability('moodle/course:update', $context_course, $user)) {
            ...
            $login_url = skyroom_getLoginUrl2($skyroom, $skyroom_user, 3, $user);
        } else if(skyroom_isTeacher($course)) {
            //teacher
            $login_url = skyroom_getLoginUrl2($skyroom, $skyroom_user, 2, $user);
        } else {
            //student
            $login_url = skyroom_getLoginUrl2($skyroom, $skyroom_user, 1, $user);
        }
        $ok_user[] = $user;
    } catch (Exception $e) {
        $nok_user[] = $user;
